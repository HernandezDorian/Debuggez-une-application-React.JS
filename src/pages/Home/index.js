...
const Page = () => {
  const { data } = useData();
  const event = data?.events || []; // Importation des events avec comme sécurité  un remplacement par tableau vide si non trouvé afin d'éviter de créer des erreurs

  const last = event.sort((a, b) => new Date(b.date) - new Date(a.date))[0]; // Tri par date afin d'avoir l'évenement le plus récent de data.event
      ...
      <main>
        ...
        <section className="ServicesContainer">
          <h2 id="nos-services" className="Title">
            {/* Ajout d'un ID pour le menu de navigation */}
            Nos services
          </h2>
